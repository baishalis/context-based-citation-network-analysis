...
public class MetricsCutRatioDirect {
	...
	public static class InputMapper extends TableMapper<Text, Text> {
		...
		protected void map(ImmutableBytesWritable row, Result value,
			...
			String fromComm = Bytes.toString(value.getValue(Bytes.toBytes("from"), Bytes.toBytes("fromComm"))).trim();
			String toComm = Bytes.toString(value.getValue(Bytes.toBytes("to"), Bytes.toBytes("toComm"))).trim();
			...
			if (!fromComm.equals(toComm)) {
				context.write(new Text(fromComm), new Text(edgeCount));
			}
	...
	public static class PaperEntryReducer extends TableReducer<Text, Text, ImmutableBytesWritable> {
		...
		protected void reduce(Text forClass, Iterable<Text> counts, Context context)
			...
			put.add(Bytes.toBytes("metricsDirect"), Bytes.toBytes("cutRatio"), Bytes.toBytes(Double.toString(cut)));
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = new Job(conf, "CutRatio");

		job.setJarByClass(MetricsCutRatioDirect.class);
		...
		TableMapReduceUtil.initTableMapperJob("CommunityNetworkForDirect", scan, InputMapper.class, Text.class, Text.class, job);
