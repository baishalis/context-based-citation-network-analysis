...
public class PaperMetricIndegreeOutdegreePatch {
	...
	public static class InputMapper extends TableMapper<Text, Text> {
		...
		protected void map(ImmutableBytesWritable row, Result value,
			...
			if (value.getValue(Bytes.toBytes("metricsIndirect"), Bytes.toBytes("indegree")) == null) {
				context.write(new Text(new String(value.getRow())), new Text("in"));
			}
			if (value.getValue(Bytes.toBytes("metricsIndirect"), Bytes.toBytes("outdegree")) == null) {
	...
	public static void main(String[] args) throws Exception {
		...
		TableMapReduceUtil.initTableMapperJob("PaperbagOfPhrases", scan, InputMapper.class, Text.class, Text.class,
				job);
